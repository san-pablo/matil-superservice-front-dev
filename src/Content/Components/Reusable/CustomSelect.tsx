...
const CustomSelect = <T extends string | number>({options, selectedItem, setSelectedItem, hide, updateData=() => {},  labelsMap=null ,iconsMap=null, containerRef, isDisabled = false, disabledOptions, includeNull = false, variant = 'common', fontSize = '.8em'}: CustomSelectProps<T>) => {
    ...
    return(
        <Box position={'relative'} w='100%' fontSize={fontSize}>
            ...
            {variant === 'common' ? 
            <Flex h='32px' bg={isDisabled ? 'brand.gray_1':'transaprent'} transition={'border-color .2s ease-in-out, box-shadow .2s ease-in-out'} cursor={isDisabled ? 'not-allowed':'pointer'} alignItems={'center'} ref={buttonRef}  onClick={()=>{if (!isDisabled) setShowList(!showList)}} justifyContent={'space-between'} p={'7px'} borderRadius='.5rem'   boxShadow={ showList ? '0 0 0 2px rgb(59, 90, 246)' : ''} border={showList ? '1px solid rgb(59, 90, 246)':hide ? '1px solid transparent' : '1px solid #E2E8F0'} _hover={{border:showList ? '1px solid rgb(59, 90, 246)':'1px solid #CBD5E0'}}>
