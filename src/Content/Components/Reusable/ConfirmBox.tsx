...
const ConfirmBox = memo(({ children, setShowBox,  maxW, max = false, upPosition = false }:ConfirmBoxProps) => {
    ...
    return(
    <Portal> 
        <MotionBox id='custom-portal' initial={{opacity:0}}  animate={{opacity:1}} display={'flex'} exit={{opacity:0}} transition={{ duration: '.2' }} onMouseDown={() => setShowBox(false)} position='fixed' alignItems='center'justifyContent='center' top={0} left={0} width='100vw' height='100vh' bg='rgba(0, 0, 0, 0.3)' backdropFilter={'blur(1px)'} zIndex= {upPosition?10001:10000}>
