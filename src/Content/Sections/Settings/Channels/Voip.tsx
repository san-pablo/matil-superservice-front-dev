...
function Voip () {
        ...
        if (response?.status === 200){
          ...
          if (voipChannels) {
            if (voipChannels.length === 1) {
                const responseMail = await fetchData({endpoint:`${auth.authData.organizationId}/admin/settings/channels/${voipChannels[0].id}`,  setValue: setData, getAccessTokenSilently, auth})
    ...
    const saveChanges = async () => {
        const response = await fetchData({endpoint:`${auth.authData.organizationId}/admin/settings/channels/${dataRef?.current?.id}`, setValue:setWaitingSend, setWaiting:setWaitingSend, auth,getAccessTokenSilently, method:'put', requestForm:{...data, matilda_configuration_uuid:selectedConfigId}, toastMessages:{'works':t('CorrectUpdatedInfo'), 'failed':t('FailedUpdatedInfo')}})
        if (response?.status === 200) {
    ...
    const selectChannel = async (id:string) => {
        const responseMail = await fetchData({endpoint:`${auth.authData.organizationId}/admin/settings/channels/${id}`,  setValue: setData,getAccessTokenSilently, auth})
        if (responseMail?.status === 200) {
            dataRef.current = responseMail.data
            setSelectedConfigId(responseMail.data.matilda_configuration_uuid)
            configIdRef.current = responseMail.data.matilda_configuration_uuid
    ...
    return(<>
        ...
        {(data === null) ?
            ...
            <Skeleton isLoaded={ channelsList !== null}>
                ...
                <Table data={channelsList || []} CellStyle={CellStyle} excludedKeys={['uuid',' id', 'channel_type']} onClickRow={(row) => selectChannel(row.id)} columnsMap={{'name':[t('Name'), 300], 'display_id':[t('Account'), 300], 'is_active':[t('ActiveChannel'), 100]}} noDataMessage='' />
            </Skeleton>
            </Box>
        :
        <>
            ...
            <Flex flex='1' overflow={'hidden'} width={'100%'} gap='5vw'> 
                <Box flex='1' pt='4vh' overflow={'scroll'}> 
                    <Skeleton isLoaded={data !== null}> 
                        ...
                        <Text mb='1vh' color='gray.600' fontSize={'.9em'}>{t('AllowInterruptionDes')}</Text>
                        ...
                        <Text mb='1vh' color='gray.600' fontSize={'.9em'}>{t('EnableTranscriptionDes')}</Text>
                        ...
                        <Text mb='1vh' color='gray.600' fontSize={'.9em'}>{t('UseAcceptanceUtterancesDes')}</Text>
                        ...
                        <Text mb='1vh'  color='gray.600' fontSize={'.9em'}>{t('UseWaitingUtterancesDes')}</Text>
...
const EditNumber = ({inputType, title, description, value, setValue, min, max, showStepper = true, step }: EditNumberType) => {
    ...
    return (<> 
        ...
        {description && <Text  color={'gray.600'} fontSize={'.9em'}>{description}</Text>}
        <NumberInput   step={step} width={'200px'} size='sm' mt='.5vh' value={currentValue} onBlur={handleBlur} onChange={(value) => setCurrentValue(value) } min={min} max={max}>
            <NumberInputField      fontSize={'.9em'} borderRadius='.5rem'   borderColor={'gray.300'} _hover={{ border: '1px solid #CBD5E0' }} _focus={{ borderColor: 'rgba(59, 90, 246)', borderWidth: '2px', px:'6px' }} px='7px' />
...
const StrList = ({title, description, values, setValues}:EditStrListType) => {
    ...
    <Box width={'100%'} mt='2vh' > 
        {values?.map((option, index) => (
            <Flex maxW={'90%'} key={`option-${index}`} mt={index === 0?'0':'1vh'} justifyContent={'space-between'}  shadow='sm' p='5px' borderRadius='.5rem' borderColor="gray.200" borderWidth="1px"  bg='gray.50'>
