...
function Instagram () {
    ...
    const fetchInitialData = async() => {
        ...
        const response = await fetchData({endpoint:`${auth.authData.organizationId}/admin/settings/channels`, auth,getAccessTokenSilently})
         if (response?.status === 200){
          ...
          if (instaChannel) {
            const responseMail = await fetchData({endpoint:`${auth.authData.organizationId}/admin/settings/channels/${instaChannel}`,  setValue: setData, auth, getAccessTokenSilently})
    ...
    const callNewWInstagram = async() => {
        const response = await fetchData({endpoint:`${auth.authData.organizationId}/admin/settings/channels/instagram`,  setValue: setData, auth, getAccessTokenSilently})
    }
