...
const ConversationsData = () => {
    ...
    useEffect(() => {        
        ...
        const fetchInitialData = async() => {
            const response = await fetchData({endpoint:`${auth.authData.organizationId}/settings/data`, setRef:conversationsDataRef, getAccessTokenSilently,setValue:setConversationsData, auth})
        }
        fetchInitialData()
     }, [])
...
return(<>
        ...
        <Box px='2vw' pt='2vh' justifyContent={'space-between'} alignItems={'end'}> 
                ...
                <Text color='text_gray' fontSize={'.8em'}>{t('ConversationsDes')}</Text>
            ...
            <Box overflow={'scroll'} flex='1' pb='2vh' px='2vw' > 
                <Skeleton   isLoaded={conversationsData !== null} width={'100%'} maxW={'1000px'} minW={'500px'}> 
                    ...
                    <Text mb='.5vh' fontSize={'.8em'} color='text_gray'>{t('DaysToCloseDes')}</Text>
                    ...
                    <Text mb='.5vh' fontSize={'.8em'} color='text_gray'>{t('InactivityDaysToCloseDes')}</Text>
                    <Box maxW={'350px'}px='2px'> 
                    ...
                    <Text mt='.5vh' fontSize={'.8em'} color='text_gray'>{t('MoveTrashDes')}</Text>

                    {conversationsData?.auto_move_to_bin_closed && <>   
                        <Text mt='3vh' fontWeight={'medium'}>{t('DaysToDelete')}</Text>
                        <Text   mb='.5vh' fontSize={'.8em'} color='text_gray'>{t('DaysToDelete')}</Text>
                        <Box maxW={'350px'}px='2px'> 
                    ...
                    {conversationsData?.auto_delete_bin && <>   
                        ...
                        <Text mb='.5vh' fontSize={'.8em'} color='text_gray'>{t('DaysToDeleteBinDes')}</Text>
                        <Box maxW={'350px'}px='2px'> 
