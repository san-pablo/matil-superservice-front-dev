...
function Triggers ({scrollRef}:{scrollRef:RefObject<HTMLDivElement>}) {
    ...
    useEffect(() => {
        const fetchTriggerData = async () => {
            const response  = await fetchData({endpoint:`${auth.authData.organizationId}/settings/triggers`, getAccessTokenSilently,setValue:setTriggerData, auth})
        }
    ...
    const DeleteComponent = () => {
        ...
        const deleteTrigger= async () => {
            ...
            const response = await fetchData({endpoint: `${auth.authData.organizationId}/settings/triggers`,getAccessTokenSilently, requestForm: newTriggers, method: 'put', setWaiting: setWaitingDelete, auth, toastMessages: {works: t('CorrectDeletedTrigger'), failed: t('FailedDeletedTrigger')}})
        ...
        return(<>
            ...
            <Flex bg='gray.50' borderTopColor={'border_color'} borderTopWidth={'1px'} p='20px' gap='10px' flexDir={'row-reverse'}>
        ...
        <Box px='2vw' py='2vh'> 
            <Flex justifyContent={'space-between'} alignItems={'end'}> 
                <Box> 
                    ...
                    <Text color='text_gray' fontSize={'.9em'}>{t('TriggersDes')}</Text>
                </Box>
            ...
            <Skeleton isLoaded={triggerData !== null}> 
                <Table data={filteredTriggerData} CellStyle={CellStyle} columnsMap={{'name':[t('Name'), 150], 'description':[t('Description'), 350]}} noDataMessage={t('NoTriggers')} excludedKeys={['all_conditions', 'any_conditions', 'actions']} onClickRow={(row, index) => setSelectedIndex(index)} />  
            </Skeleton>
        </Box> 
        </>}
    </>)
...
const EditTrigger = ({triggerData, selectedIndex, setSelectedIndex, allTriggers, setAllTriggers, scrollRef }:{triggerData:ActionDataType, selectedIndex:number, setSelectedIndex:Dispatch<SetStateAction<number >>,allTriggers:ActionDataType[], setAllTriggers:Dispatch<SetStateAction<ActionDataType[] | null>>, scrollRef:RefObject<HTMLDivElement>}) => {
    ...
    const sendTrigger = async () => {
        ...
        const response = await fetchData({endpoint:`${auth.authData.organizationId}/settings/triggers`, requestForm:newAutomations, method:'put',getAccessTokenSilently, setWaiting:setWaitingSend, auth, toastMessages:{works:selectedIndex === -1?t('CorrectCreatedTrigger'):t('CorrectUpdatedTrigger'), failed: selectedIndex === -1?t('FailedCreatedTrigger'):t('FailedUpdatedTrigger')}})
        if (response?.status === 200) {
            setSelectedIndex(-2)
        ...
        <Box px='2vw' py='2vh'> 
            <Flex alignItems={'end'} justifyContent={'space-between'}>
                <Flex fontWeight={'medium'} fontSize={'1.4em'} gap='10px' alignItems={'center'}> 
                    <Text onClick={() => setSelectedIndex(-2)}  color='text_blue' cursor={'pointer'}>{t('Triggers')}</Text>
