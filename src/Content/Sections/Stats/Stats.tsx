...
function Stats () {
    ...
    //CONSTANTS
    const auth = useAuth()
    const sectionsMap: Record<IconKey, [string, IconType]> = {tickets: ['Tickets',FaTicket], matilda: ['Matilda',FaRobot], users: ['Usuarios',IoPeople]}
    ...
    useEffect(() => {
        ...
        document.title = `Estadísticas - ${sectionsMap[selectedSection][0]} - ${auth.authData.organizationName} - Matil`
        localStorage.setItem('currentSection', `stats`)

        navigate(selectedSection)
        if (selectedSection) {
            setSelectedChannels(sectionsFilters[selectedSection].channel_types)
    ...
    //CURRENT CHANNELS LOGIC
    const [selectedChannels, setSelectedChannels] = useState<string[]>([])  
    const toggleChannelsList = (channel:string) => {
        if (selectedChannels.includes(channel)) setSelectedChannels(selectedChannels.filter(c => c !== channel))
         else setSelectedChannels([...selectedChannels, channel])     
    }
            ...
            <Box > 
                <Flex justifyContent={'space-between'} alignItems={'end'}> 
                    ...
                    <Flex gap='20px'> 
                        ...
                        {/*
                        <FilterButton selectList={Object.keys(logosMap)} selectedElements={selectedChannels} setSelectedElements={(element) => toggleChannelsList(element as Channels)} icon={PiDesktopTowerFill} filter='channels'/>
                        <Button bg='brand.gradient_blue' onClick={() => setSectionFilters(prevFilters => ({...prevFilters, [selectedSection]: {...prevFilters[selectedSection],'channel_types': selectedChannels}}))} _hover={{bg:'brand.gradient_blue_hover'}} color='white' height={'31px'} size='sm'>Aplicar filtros</Button>
                        */}
                    </Flex>
                </Flex>
            </Box> 
