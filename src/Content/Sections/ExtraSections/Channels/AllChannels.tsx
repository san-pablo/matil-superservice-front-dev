...
import { logosMap, Channels, ChatBotData } from '../../../Constants/typing'
...
const AllChannels = () => {
    ...
    const CreateChannel = () => {
        ...
        const createChannel = async (channelConfig?:any) => {
            const endpoint = `${auth.authData.organizationId}/settings/channels/${channelType}`
            let chanelConfig:any  = {}
            if (channelType === 'webchat')  chanelConfig = {name, configuration:defaultChatbotConfig}
            else if (channelType === 'email')  chanelConfig = {name, email_address:email}
            else if (channelType === 'instagram')  chanelConfig = {name, ...channelConfig}
            else if (channelType === 'google_business')  chanelConfig = {name, ...channelConfig}
            else if (channelType === 'whatsapp')  chanelConfig = {name, ...channelConfig}

            const response = await fetchData({endpoint, setWaiting:setWaitingCreate, getAccessTokenSilently, auth, method:'post', requestForm:chanelConfig, toastMessages:{'works':t('CorrectUpdatedInfo'), 'failed':t('FailedUpdatedInfo')}})
        }   
        ...
        const memoizedCreateBox = useMemo(() => (
            <ConfirmBox setShowBox={() =>  navigate(`/channels/${currentSearch}`)} >
    ...
    useEffect(() => {
        const fetchChannels = async() => {
            const response = await fetchData({endpoint:`${auth.authData.organizationId}/settings/channels`, auth, setValue:setChannelsData ,getAccessTokenSilently})
        }
        fetchChannels()
    ...
    return (<> 
        ...
        <Box px='2vw' py='2vh' > 
            <Flex alignItems={'end'} justifyContent={'space-between'}> 
                <Box> 
                    <Text fontSize={'1.4em'} fontWeight={'medium'}>{t('Channels')}</Text>
                 </Box>
                <Button size='xs' leftIcon={<FaPlus/>} variant={'main'} onClick={() => setShowCreateChannel(true)}>{t('CreateChannel')}</Button>
            </Flex>
             
 
            <Box mt='2vh' width={'100%'}> 
                <Table data={channelsData} CellStyle={CellStyle} excludedKeys={['id']} onClickRow={(row) => navigate(`/channel/${row.type}/${row.id}${currentSearch}`)} columnsMap={{ 'name':[t('Name'), 300], 'channel_type':[t('ChannelType'), 200],  'display_id':[t('Account'), 300], 'is_active':[t('Status'), 200]}} noDataMessage={t('NoChannels')} />
            </Box>
        </Box>
    </>)
}
...
const SuccessPage = ({ callNewData}:{callNewData:(data:any) => void}) => {
    ...
    const sendData = async () => {     
        ...
        navigate(`/channels/${currentSearch}`)
    }
