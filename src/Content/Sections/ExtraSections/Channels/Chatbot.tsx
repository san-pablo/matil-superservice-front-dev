...
function Chatbot () {
    ...
    const newSection = {
        ...
        options:{[i18n.language.toLocaleUpperCase()]:[] as string[]},
    }
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async() => {
            ...
            const responseChat = await fetchData({endpoint:`${auth.authData.organizationId}/settings/channels/${channelId}`,setValue:setChannelDict, getAccessTokenSilently, auth, })
            if (responseChat?.status === 200) { 
                ...
                configIdRef.current = responseChat.data.matilda_configuration_id
                setSelectedConfigId(responseChat.data.matilda_configuration_id)
            }
        }
        fetchInitialData()
    }, [])
    ...
    const duplicateChannel = async() => {
        const response = await fetchData({endpoint:`${auth.authData.organizationId}/settings/channels/webchat`, getAccessTokenSilently, auth, method:'post', requestForm:{name:channelDict.name + ' ' + t('Copy'),  configuration:chatBotData}, toastMessages:{'works':t('CorrectCopiedChannel'), 'failed':t('FailedCopiedChannel')}})
        if (response?.status === 200) {
            const newChannel = {id: response.data.id, display_id: `webchat_${response.data.id}`, name: channelDict.name + ' ' + t('Copy'), channel_type: 'webchat', is_active: true}
    ...
    const sendChatBotCofig = async() => {
        ...
        const response = await fetchData({endpoint:`${auth.authData.organizationId}/settings/channels/${channelId}`, getAccessTokenSilently, auth, method:'put', requestForm:{...channelDict, configuration:{...chatBotData, chat_avatar:chatAvatar, messages_opacity:opacity}, matilda_configuration_uuid:selectedConfigId}, toastMessages:{'works':t('CorrectUpdatedInfo'), 'failed':t('FailedUpdatedInfo')}})
        if (response?.status === 200) {
    ...
    const memoizedShowCodeBox = useMemo(() => (
        <ConfirmBox setShowBox={setShowCode}> 
            <Box p='15px'>
                ...
                <Text  mb='2vh' fontSize={'.9em'} color='text_gray'>{t('InsertCodeDes')}</Text>
                <Flex p='15px' gap='20px'   borderRadius={'.5rem'} bg='gray_2'>
                    <Text flex='1' color='text_gray' fontSize={'.7em'}>{`<script defer src="https://chat-widget-matlil.s3.eu-west-3.amazonaws.com/insert.js?organization_id=${auth.authData.organizationId}&chatbot_id=${channelDict?.id}" id="matil-chat-widget"></script>`}</Text>
                </Flex>
    ...
    const memoizedDeleteBox = useMemo(() => {
        ...
        const handleDeleteChannel= async() => {
            const response = await fetchData({endpoint:`${auth.authData.organizationId}/functions/${channelId}`, getAccessTokenSilently, setWaiting:setWaitingDelete, method:'delete', auth, toastMessages:{'works':t('CorrectDeletedChannel'), 'failed':t('FailedDeletedChannel')}})
            if (response?.status == 200) {
        ...
        <ConfirmBox setShowBox={setShowConfirmDelete}> 
                <Box p='15px'> 
                    ...
                    <Text mt='2vh' fontSize={'.8em'}  color='text_gray'>{t('DeleteChannelWarning')}</Text>
                            ...
                            return (<>
                              <Skeleton isLoaded={configData !== null}> 
                                    ...
                                    {configData?.map((config, index) => (
                                        <Box transition={'box-shadow 0.2s ease-in-out'} _hover={{shadow:'md'}}  mt='1vh' key={`config-${index}`} bg={selectedConfigId === config.id?'gray_2':'white'} onClick={() => setSelectedConfigId(config.id)} borderColor={'border_color'} borderWidth={'1px'} borderRadius={'.5rem'} p='15px' cursor={'pointer'}>
                            ...
                            return (<>
                                ...
                                <Flex mt='3vh'> 
                                    <Flex gap='10px' cursor={'pointer'}  alignItems={'center'} onClick={(e) => setChatBotData({...chatBotData as ChatBotData, 'chat_position':'left'})}> 
                                        ...
                                        <Icon  boxSize={'20px'} color={chatBotData?.chat_position === 'left' ? 'text_blue':''} as={BsThreeDots}/>
                                        <Flex  height={'20px'} borderRadius={'50%'} width={'20px'}  bg={chatBotData?.chat_position === 'left' ? 'text_blue':''} alignItems={'center'} justifyContent={'center'}  borderColor={'border_color'} borderWidth={'1px'}>
                                    ...
                                    <Flex gap='10px' cursor={'pointer'} alignItems={'center'} onClick={(e) => setChatBotData({...chatBotData as ChatBotData, 'chat_position':'right'})}> 
                                        <Flex  height={'20px'} borderRadius={'50%'} width={'20px'}  bg={chatBotData?.chat_position === 'right' ? 'text_blue':''} alignItems={'center'} justifyContent={'center'}  borderColor={'border_color'} borderWidth={'1px'}>
                                            <Box borderRadius={'50%'} bg='white' h='7px' w='7px'/>
                                        </Flex>
                                        <Icon boxSize={'20px'} color={chatBotData?.chat_position === 'right' ? 'text_blue':''} as={BsThreeDots}/>
                            ...
                            return (<>
                                ...
                                <Box width={'100%'} mt='1vh'> 
                                    {(chatBotData?.options[selectedLanguage] || []).map((option, urlIndex) => (
                                        <Flex key={`option-${urlIndex}`} mt={urlIndex === 0?'0':'1vh'} justifyContent={'space-between'} alignItems={'center'} p='5px' borderRadius=".5em" borderColor="border_color" borderWidth="1px" bg="hover_gray">
...
const EditSection = ({chatBotData, setChatBotData, index, expandedIndex, setExpandedIndex}:{chatBotData:ChatBotData, setChatBotData:Dispatch<SetStateAction<ChatBotData | null>>,  index:number, expandedIndex:number, setExpandedIndex:Dispatch<SetStateAction<number>>}) => {
    ...
    <Box p='20px'  bg={(isExpanded || isHovering) ?'white':'#FDFDFD'}  width={'100%'} mb='3vh'  borderWidth={'1px'} borderColor={(isExpanded || isHovering) ? 'text_blue':'border_color'} borderRadius={'.5rem'} shadow={(isExpanded || isHovering)?'md':'sm'} transition={'box-shadow 0.3s ease-in-out, background-color 0.3s ease-in-out,border-color 0.3s ease-in-out'} onMouseEnter={() => setIsHovering(true)}  onMouseLeave={() => setIsHovering(false)}  >
        ...
        <Box alignItems={'center'} justifyContent={'space-between'}  >
            ...
            <Text fontSize={'.8em'} color='text_gray'>{t('AllowedUrlsDes')}</Text>
            <Box width={'100%'} mt='2vh'> 
                {section?.allowed_urls.map((option, index2) => (
                    <Flex key={`option-${index2}`} mt={index2 === 0?'0':'1vh'} justifyContent={'space-between'} alignItems={'center'} p='5px' borderRadius=".5em" borderColor="border_color" borderWidth="1px" bg="gray_2">
                        <Text fontSize={'.9em'}>{option}</Text>
        ...
        <motion.div initial={false} animate={{height:isExpanded?'auto':0}} exit={{height:isExpanded?0:'auto'}} transition={{duration:.3}} style={{overflow:'hidden'}} >           
            ...
            <Text mt='.5vh' fontSize={'.8em'} color='text_gray'>{t('ActivateFloatingMessagesDes')}</Text>
            ...
            <Text mt='.5vh' fontSize={'.8em'} color='text_gray'>{t('ShowEnterMessageDes')}</Text>
            ...
            <Text mt='.5vh' fontSize={'.8em'} color='text_gray'>{t('ShowOnClickDes')}</Text>
            ...
            {(section.lure_by_click || section.lure_by_seconds) && <>
                ...
                <Text fontSize={'.8em'} color='text_gray'>{t('WelcomeMessageDes')}</Text>
                <Box width='100%' mt='1vh'> 
                ...
                <Text fontSize={'.8em'} color='text_gray'>{t('ShortcutWebsDes')}</Text>
                <Box width={'100%'} mt='2vh'> 
                    {section?.options[selectedLanguage].map((option, index3) => (
                        <Flex key={`option-${index3}`} mt={index3 === 0?'0':'1vh'} justifyContent={'space-between'} alignItems={'center'} p='5px' borderRadius=".5em" borderColor="border_color" borderWidth="1px" bg="gray_2">
                            <Text fontSize={'.9em'}>{option}</Text>
...
const ShowChatBox = ({chatBotData, selectedLanguage}:{chatBotData:ChatBotData | null, selectedLanguage:string}) => {
    ...
    return (
        ...
        <Flex flexDir={'column'} h='100%' borderRadius={'.7rem'}  flex='1' bg='gray_2' position={'relative'}  overflow={'scroll'}> 
            
            <Flex py='2vh' px='2vh' flexDir={'column'} justifyContent={'end'} alignItems={chatBotData?.chat_position === 'right' ?'end':'start'} flex='1'  overflow={'scroll'}> 
                <Skeleton isLoaded={(chatBotData !== null)}> 
