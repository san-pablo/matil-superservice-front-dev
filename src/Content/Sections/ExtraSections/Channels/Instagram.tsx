...
function Instagram () {
    ...
    const fetchInitialData = async() => {
        await fetchData({getAccessTokenSilently, endpoint:`${auth.authData.organizationId}/settings/matilda_configurations`, setValue:setConfigData, auth})
    ...
    useEffect(() => {
      ...
      const saveChanges = async () => {
        const response = await fetchData({endpoint:`${auth.authData.organizationId}/settings/channels/${channelId}`, setValue:setWaitingSend, setWaiting:setWaitingSend,getAccessTokenSilently, auth, method:'put', requestForm:{...data, matilda_configuration_id:selectedConfigId !== ''?selectedConfigId:null}, toastMessages:{'works':t('CorrectUpdatedInfo'), 'failed':t('FailedUpdatedInfo')}})
                    ...
                    {configData?.map((config, index) => (
                        <Box transition={'box-shadow 0.3s ease-in-out'} _hover={{shadow:'md'}}  mt='2vh' key={`config-${index}`} bg={selectedConfigId === config.id?'rgba(59, 90, 246, 0.25)':'gray.50'} onClick={() => setSelectedConfigId(config.id)} borderColor={'border_color'} borderWidth={'1px'} borderRadius={'.5rem'} p='15px' cursor={'pointer'}>
                            <Flex justifyContent={'space-between'}> 
                                ...
                                {selectedConfigId === config.id && <IconButton size={'xs'} color={'red.600'} bg='transparent' variant={'delete'} _hover={{bg:'red.100'}} icon={<RxCross2 size={'16px'}/>} aria-label="delete-row" onClick={(e) => {e.stopPropagation();setSelectedConfigId('')}}/>}
                            </Flex>
                        </Box> 
                    ))}
                </Skeleton>           
